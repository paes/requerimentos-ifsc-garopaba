...
<aside class="w-64 bg-white shadow-xl z-40 flex flex-col h-full border-r border-gray-100 hidden md:flex">
    ...
    <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        ...
        <?php if ($isSysAdmin): ?>
            ...
            <a href="teachers.php"
                class="flex items-center px-4 py-3 rounded-lg text-sm font-medium transition-all duration-200 group <?= $currentPage == 'teachers.php' ? 'bg-brand-DEFAULT/10 text-brand-DEFAULT' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' ?>">
                <svg class="w-5 h-5 mr-3 <?= $currentPage == 'teachers.php' ? 'text-brand-DEFAULT' : 'text-gray-400 group-hover:text-gray-500' ?>"
                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
