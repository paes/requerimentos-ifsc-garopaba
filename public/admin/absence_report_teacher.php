<?php
/**
 * Relatorio gerencial de justificativas de faltas agrupadas para os professores.
 *
...
$pageTitle = 'Relatório de Justificativas de Faltas (Professor)';
                ...
                <div class="flex flex-col md:flex-row justify-between items-center mb-6">
                    <div>
                        <h2 class="text-2xl font-bold text-gray-800 tracking-tight">Relatório de Justificativas de
                            Faltas (Professor)</h2>
                        <p class="text-gray-500 mt-1">Filtragem e impressão de justificativas (Versão Professor)</p>
                    </div>
                    ...
                    <table class="w-full whitespace-nowrap">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <?php if (empty($requests)): ?>
                                <tr>
                                    <td colspan="5" class="px-6 py-8 text-center text-gray-500">Nenhuma justificativa
                                        encontrada.</td>
                                </tr>
                            <?php else: ?>
                                <?php foreach ($requests as $r): ?>
                                    <tr class="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td class="px-6 py-4 text-sm text-gray-500 max-w-xs truncate"
                                            title="<?= htmlspecialchars($r['course_units'] ?? '') ?>">
                                            <?= htmlspecialchars($r['course_units'] ?? '-') ?>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php endif; ?>
...
<div class="hidden print:block p-8 bg-white">
    <div class="text-center mb-8 border-b pb-4">
        ...
        <h1 class="text-2xl font-bold text-gray-900">Relatório de Justificativas de Faltas (Professor)</h1>
        <p class="text-gray-600">Semestre:
            <?= htmlspecialchars($semester) ?>
        </p>
        <p class="text-gray-500 text-sm mt-1">Gerado em:
            <?= date('d/m/Y H:i') ?>
        </p>
    </div>
    ...
    <table class="w-full border-collapse border border-gray-300 table-fixed">
        <thead>
            <tr class="bg-gray-100">
                <th class="border border-gray-300 px-4 py-2 text-left text-sm font-bold text-gray-700 w-[20%]">Aluno
                </th>
                <th class="border border-gray-300 px-4 py-2 text-left text-sm font-bold text-gray-700 w-[15%]">Turma
                </th>
                <th class="border border-gray-300 px-4 py-2 text-left text-sm font-bold text-gray-700 w-[15%]">D. Início
                </th>
                <th class="border border-gray-300 px-4 py-2 text-left text-sm font-bold text-gray-700 w-[15%]">D. Final
                </th>
                <th class="border border-gray-300 px-4 py-2 text-left text-sm font-bold text-gray-700 w-[35%]">Und.
                    Curriculares
                </th>
            </tr>
        </thead>
