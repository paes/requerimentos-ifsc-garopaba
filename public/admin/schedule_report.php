<?php
/**
 * Relatorio focado nas solicitacoes de entrada/saida fora do horario padrao.
 *
...
// Controle de Acesso: Perfil 2 (Coordenador) OU Admin (Perfil 1 implicito pelo acesso admin/sidebar)
// Com base na logica da sidebar, vamos permitir Perfil 2 e checar isSysAdmin
$db = new Database();
...
if ($user['user_role'] != 2 && !$isSysAdmin) {
...
$query = "
    ...
    WHERE r.request_type_id = 20 AND r.status IN ('approved', 'concluded')
";
$params = [];
...
$pageTitle = 'Relatório de Horário Diferenciado';
        ...
        <div class="container mx-auto">
            ...
            <div class="bg-white rounded-2xl shadow-sm p-6 mb-6 print:hidden">
                <div class="flex flex-col md:flex-row justify-between items-center mb-6">
                    <div>
                        <h2 class="text-2xl font-bold text-gray-800 tracking-tight">Relatório de Horário Diferenciado
                        </h2>
                        <p class="text-gray-500 mt-1">Filtragem e impressão de solicitações de horário diferenciado</p>
                    </div>
                    ...
                    <table class="w-full whitespace-nowrap">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <?php if (empty($requests)): ?>
                                <tr>
                                    <td colspan="5" class="px-6 py-8 text-center text-gray-500">Nenhuma solicitação
                                        encontrada.</td>
                                </tr>
                            <?php else: ?>
                                    ...
                                    <tr class="hover:bg-gray-50 transition-colors">
                                        <td class="px-6 py-4 text-sm font-medium text-gray-900">
                                            ...
                                            <div class="text-xs text-gray-500 font-normal mt-0.5">
                                                <?= htmlspecialchars($r['course_name'] ?? '') ?></div>
                                        </td>
                                        <td class="px-6 py-4 text-sm text-gray-500">
                                            <?= htmlspecialchars($r['class_info'] ?? '-') ?></td>
                                        <td class="px-6 py-4 text-sm text-gray-500"><?= htmlspecialchars($r['schedule_type']) ?>
                                        </td>
                                        <td class="px-6 py-4 text-sm text-gray-500">
                                            <?= $r['arrival_time_1'] ? substr($r['arrival_time_1'], 0, 5) : '-' ?>
                                            <?= $r['arrival_time_2'] ? ' / ' . substr($r['arrival_time_2'], 0, 5) : '' ?>
                                        </td>
                                        <td class="px-6 py-4 text-sm text-gray-500">
                                            <?= $r['departure_time_1'] ? substr($r['departure_time_1'], 0, 5) : '-' ?>
                                            <?= $r['departure_time_2'] ? ' / ' . substr($r['departure_time_2'], 0, 5) : '' ?>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php endif; ?>
...
<div class="hidden print:block p-8 bg-white">
    <div class="text-center mb-8 border-b pb-4">
        <img src="../assets/img/logob.png" alt="IFSC" class="h-12 mx-auto mb-2 brightness-0 filter grayscale">
        <h1 class="text-2xl font-bold text-gray-900">Relatório de Horário Diferenciado</h1>
    ...
    <table class="w-full border-collapse border border-gray-300">
        <thead>
            <tr class="bg-gray-100">
                <th class="border border-gray-300 px-4 py-2 text-left text-sm font-bold text-gray-700">Aluno</th>
                <th class="border border-gray-300 px-4 py-2 text-left text-sm font-bold text-gray-700">Turma</th>
                <th class="border border-gray-300 px-4 py-2 text-left text-sm font-bold text-gray-700">Tipo</th>
                <th class="border border-gray-300 px-4 py-2 text-left text-sm font-bold text-gray-700">Chegada</th>
                <th class="border border-gray-300 px-4 py-2 text-left text-sm font-bold text-gray-700">Saída</th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($requests as $r): ?>
                <tr>
                    ...
                    <td class="border border-gray-300 px-4 py-2 text-sm"><?= htmlspecialchars($r['schedule_type']) ?></td>
                    <td class="border border-gray-300 px-4 py-2 text-sm">
                        <?= $r['arrival_time_1'] ? substr($r['arrival_time_1'], 0, 5) : '-' ?>
                        <?= $r['arrival_time_2'] ? ' / ' . substr($r['arrival_time_2'], 0, 5) : '' ?>
                    </td>
